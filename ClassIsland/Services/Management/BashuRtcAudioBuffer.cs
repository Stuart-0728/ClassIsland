...
public sealed class BashuRtcAudioBuffer : ISoundDataProvider
{
    ...
    public void Push(ReadOnlySpan<float> mono)
    {
        lock (Gate)
        {
            ...
            if (!Enabled) { Samples.Clear(); Primed = false; return; }
    ...
    public int ReadBytes(Span<float> buffer)
    {
        ...
        lock (Gate)
        {
            ...
            if (!Enabled) { Samples.Clear(); Primed = false; return buffer.Length; }
            if (!Primed && Samples.Count >= 5760) Primed = true; // 60 ms initial cushion.
            if (Primed)
            {
                var count = Math.Min(buffer.Length, Samples.Count);
                for (var i = 0; i < count; i++) buffer[i] = Samples.Dequeue();
                if (count < buffer.Length) Primed = false;
            }
